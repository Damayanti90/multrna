...
node_dic={}
symb="`"
fs=open(r"merged_embeddings_mir_go_dis_BiGRU.txt","r")
for line in fs:
    count+=1
    x=line.split(symb)
    node_lst.append(x[0])
...
symb="`"
edge_lst=[]
fs=open(r"total_ID_mgd_BiGRU_jaccard.txt","r")
for line in fs:
    count+=1
    x=line.split(symb)
...
count=0
edge_label=[]
fs=open(r"labels_total_ID_mgd_BiGRU_jaccard.txt","r")
for line in fs:
    count+=1
...
def objective(trial):
    ...
    skf = StratifiedKFold(n_splits=internal_fold, shuffle=True, random_state=42)
    fold_accuracies = []
...
df.to_csv("optuna_trials_log_lightGBM_BiGRU.csv", index=False)
...
for train_index, test_index in repeated_kfold.split(X, y):
    ...
    print("fold=",counter,"accuracy",round(acc,4),"precision",round(prec,4),"recall",round(rec,4),"f1-score",round(f1,4),"specificity",round(spec,4),"MCC",round(mcc,4), "AUROC", round(auroc,4), "AUPRC",round(auprc,4))
...
fs=open(r"average_performance_metrics_lightGBM_BiGRU.txt","w")
...
count=0
c=0
fs=open(r"result_test_data_lightGBM_merged.txt","w")
...
symb="`"
count=0
ft=open(r"blind_data_jaccard_155.txt","r")
pred_lst=[]
for line in ft:
...
count=0
c=0
fs=open(r"result_blind_data_lightGBM_merged.txt","w")
for i in range(len(pred_lst)):
    count+=1
    s=""
    s+=pred_lst[i]
...
with open('feature_importance_lightGBM_merged.txt', 'w') as f:
...
plt.savefig('beeswarm_plot_lightGBM_merged.png', dpi=300, bbox_inches='tight')
#plt.show()
plt.close()
